...
bool SceneGame::init(bool red)
{
    ...
    //设置描点为(0,0)
    plate->setAnchorPoint(Vec2::ANCHOR_BOTTOM_LEFT);
   ...
   //创建悔棋按钮
    ...
    cocos2d::log("x=%lf", itemStart->getPositionX());
	cocos2d::log("y=%lf", itemStart->getPositionY() - 240);
...
bool SceneGame::onTouchBegan(CCTouch* pTouch, CCEvent* pEvent)
{
    ...
    else
    {
     ...
     cocos2d::log("_selectid=%d, clickid=%d", _selectid, clickid);
...
bool SceneGame::getClickPos(CCPoint ptInWin, int &x, int &y)
{
    for(x=0; x<=8; x++)
    {
        for(y=0; y<=9; y++)
        {
            ...
            cocos2d::log("ptInPlate.x=%lf   ptInPlate.y=%lf", ptInPlate.x,  ptInPlate.y);
...
void SceneGame::moveStone(int moveId, int killId, int x, int y)
{
    ...
    cocos2d::log("killId=%d, moveId=%d", killId, moveId);
    cocos2d::log("_s[moveId]->getRed()=%d", _s[moveId]->getRed());
...
bool SceneGame::canMoveJiang(int moveid, int killid, int x, int y)
{
   ...
   cocos2d::log("x=%d, y=%d", x, y);
   cocos2d::log("moveid=%d, killid=%d", moveid, killid);
    //将的对杀
